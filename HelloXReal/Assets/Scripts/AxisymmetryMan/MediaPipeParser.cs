...
public class MediaPipeParser
{
    ...
    private static List<Vector3> ReadFrame(string frameString)
    {
        // 前後の不要な文字を削除（括弧やスペースなど）
        frameString = frameString.Trim('[', ']', ' ', '\n');

        // 各タプルの文字列を分割
        string[] tupleStrings = frameString.Split(new[] { "), (" }, StringSplitOptions.None);
    ...
    public static List<List<Vector3>> ReadSequence(string sequenceString)
    {
        string[] frameStrings = sequenceString.Split(new[] {"], ["}, StringSplitOptions.None);
        List<List<Vector3>> sequence = new List<List<Vector3>>();
        foreach (string frameString in frameStrings) {
            if (frameString != "") {    // TODO: Hundle empty frame
                sequence.Add(ReadFrame(frameString));
            }
        }
        for (int i = 0; i < sequence.Count; i++) {
            for (int j = 0; j < sequence[i].Count; j++) {
                sequence[i][j] = Vector3.Scale(sequence[i][j], new Vector3(1, -1, 1));
            }
        }
        Vector3 origin = (sequence[0][23] + sequence[0][24]) / 2;
        for (int i = 0; i < sequence.Count; i++) {
            for (int j = 0; j < sequence[i].Count; j++) {
                sequence[i][j] -= origin;
            }
